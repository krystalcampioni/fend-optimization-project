...
function updatePositions() {
  ...
  var scrollDistance = document.body.scrollTop / 1250; // saves the scroll distance in a variable outside the loop
  var phases = []; // creates an empty array to save the phases outside the loop
  for (var i = 0; i < 5; i++) {
    phases.push(Math.sin(scrollDistance + i)); // pushes the phases to the phases array
  }

  for (var i = 0; i < items.length; i++) {
    items[i].style.left = items[i].basicLeft + 100 * phases[i % 5] + 'px'; // this changes the position of the pizzas, using the phases from the array
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // saved the moving pizzas div in a variable to prevent requesting the dom several times

  for (var i = 0; i < 40; i++) { // reduced the number of pizzas
